...
function getDamageReport(basePower, armorPercent, roll) {
  const damage = calculateDamage(basePower, armorPercent);
  const isCrit = isCriticalHit(roll);
  const finalDamage = applyCritical(damage, isCrit);
...
function getRareLoot(drops) {
  return drops.filter(drop => drop.rarity !== "common");
}

function getTotalValue(drops) {
  return drops.reduce((total, drop) => total + drop.value, 0);
}

function getBestDrop(drops) {
  return drops.reduce((best, drop) =>
    drop.value > best.value ? drop : best
  );
}
...
/* =========================================================
   ...
   * getEquippedWeapon(character) should destructure the name out
     of the FIRST item in the equipment array, and return just that
     name.
...
function getMainStat(character) {
  const { strength: power } = character.stats;
  return `STR: ${power}`;
}

function getEquippedWeapon(character) {
  const [{ name }] = character.equipment;
  return name;
}
...
function buildHealthBar(currentHP, maxHP, barLength) {
  ...
  const filled = Math.round((currentHP / maxHP) * barLength);
...
function countOccurrences(log, keyword) {
  ...
  for (let i = 0; i <= log.length - keyword.length; i++) {
    if (log.indexOf(keyword, i) === i) {
      count++;
    }
  }
...
function resolveAttack(bossHP, bossMaxHP, basePower, armorPercent, roll) {
  ...
  const damage = applyCritical(calculateDamage(basePower, armorPercent), isCritical);
  const newHP = Math.max(0, bossHP - damage);
  const healthBar = buildHealthBar(newHP, bossMaxHP, 10);
  const defeated = newHP === 0;

  return { damage, newHP, healthBar, defeated };  
}
